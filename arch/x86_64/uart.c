...
struct serial_device serial_devices[4];
int serial_device_number;
...
struct serial_device *new_x86_uart(port_addr_t address) {
	struct serial_device *dev = &serial_devices[serial_device_number++];
...
static void x86_uart_setup(port_addr_t p) {
	...
	outb(p + UART_INTERRUPT_ENABLE, 0x9);
	new_x86_uart(p);
}
...
void x86_uart_init() {
	...
	irq_install(IRQ_SERIAL1, x86_uart_irq_handler, &serial_devices[0]);
	irq_install(IRQ_SERIAL2, x86_uart_irq_handler, &serial_devices[1]);
}
