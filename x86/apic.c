...
int enable_apic(size_t addr) {
        ...
        vmm_map(lapic_addr, lapic_addr,
                PAGE_WRITEABLE); // move to some sane place

        volatile uint32_t *lapic_spiv =
            (volatile uint32_t *)(lapic_addr + 0xF0);
...
int enable_lapic_timer(int per_second) { return 0; }
