...
uintptr_t vmm_virt_to_phy(uintptr_t virtual) {
        ...
        if (!(pd & PAGE_PRESENT))
                return -1;
        if (pd & PAGE_ISHUGE)
                return (pd & PAGE_MASK_4M) + (virtual & PAGE_OFFSET_4M);
...
bool vmm_map(uintptr_t virtual, uintptr_t physical, int flags) {
        ...
        if (!(*pd_entry & PAGE_PRESENT)) {
                ...
                if (virtual < KERNEL_BASE) {
                        make_next_table(pd_entry, PAGE_WRITEABLE |
                                                      PAGE_PRESENT |
                                                      PAGE_USERMODE);
                } else {
                        make_next_table(pd_entry,
...
void vmm_map_range(uintptr_t virtual, uintptr_t physical, uintptr_t len,
        ...
        for (uintptr_t i = 0; i < len; i++) {
                vmm_map(virtual + i * PAGE_SIZE, physical + i * PAGE_SIZE,
                        flags);
        }
}
...
void vmm_create_unbacked(uintptr_t vma, int flags) {
        ...
        if (flags & PAGE_PRESENT) {
                panic("vmm_create_unbacked(%#zx, %x): flags cannot include "
                      "PRESENT\n",
                      vma, flags);
        }
...
int vmm_do_page_fault(uintptr_t fault_addr) {
        ...
        if (pt & PAGE_UNBACKED && !(pt & PAGE_PRESENT)) {
                ...
                *ppt &= PAGE_FLAGS_MASK; // remove any extra bits (see
                                         // create_unbacked)
                *ppt |= phy | PAGE_PRESENT;
                ...
                memcpy(temp_page, (char *)(fault_addr & PAGE_MASK_4K),
                       PAGE_SIZE);
                ...
                *ppt |= phy | PAGE_PRESENT | PAGE_WRITEABLE;
                ...
                memcpy((char *)(fault_addr & PAGE_MASK_4K), temp_page,
                       PAGE_SIZE);
                invlpg(fault_addr);

                return 1;
        } else if (pt & PAGE_STACK_GUARD) {
