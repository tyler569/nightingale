    ...
    const char *debug, unsigned ptr_mask)
{
    ...
    if (syscall_table[number])
        return -1;
    syscall_table[number] = reinterpret_cast<uintptr_t>(fn);
    ...
    uintptr_t arg5, uintptr_t arg6)
{
    ...
    if (static_cast<size_t>(syscall_num) >= SYSCALL_TABLE_SIZE
        || syscall_num <= 0) {
        return -ENOSYS;
    }
    auto syscall
        = reinterpret_cast<sysret (*)(...)>(syscall_table[syscall_num]);
    if (!syscall)
        return -ENOSYS;
...
{
    sysret ret;
    auto syscall_num = static_cast<ng_syscall>(FRAME_SYSCALL(frame));
...
extern "C" void proc_syscalls(struct file *ofd)
{
