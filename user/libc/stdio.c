        ...
        for (size_t i = 0; i < len; i++) {
                ...
                next_char:;
                        ...
                        case 's':
                                ...
                                // Break this garbage out in to a function
                                // maybe?
                                if (format.pad.len || constrain_string_len) {
                                        size_t l = strlen(str);
                                        if (format.pad.len > l &&
                                            !constrain_string_len) {
                                                if (format.pad.direction ==
                                                    RIGHT) {
                                                        for (size_t i = 0;
                                                             i <
                                                             format.pad.len - l;
                        ...
                        if (do_print_int) {
                                ...
                                buf_ix +=
                                    format_int(&buf[buf_ix], value, format);
                        }
                } else {
                        buf[buf_ix++] = fmt[i];
                }
