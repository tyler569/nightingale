...
bool recurse_rules(struct list *rules, int indent) {
    list_for_each(struct bag_entry, b, rules, node) {
        // print_indent(indent);
        // printf("x\n");
        // printf("%zx\n", b->typ);
        if (b->typ == shiny_gold) return true;
        if (recurse_rules(b->typ, indent + 1)) return true;
    }
    return false;
}
...
void traverse_rules() {
    ...
    for (size_t i=0; i<HTBL_SIZE; i++) {
        if (htbl[i].next == 0) continue;
        tried_count += 1;

        n += recurse_rules(&htbl[i], 1);
    }
    printf("tried: %li\n", tried_count);
    // assert(tried_count == parse_count && "probably that's a hash collision");
